...
from app.backtest.utils import create_ticker_and_stock, get_stock_data_by_ticker
...
async def process_backtest(
    ...
    df = await session.run_sync(get_stock_data_by_ticker, backtest.ticker_data)
    meta_data, graph = await implement_strategy(df, backtest)
